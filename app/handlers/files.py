...
async def fix_file(repository, source_branch, file, issues):
    ...
    available_fixes = await gather(*[get_codefix_availability(issue_id=issue['key']) for issue in issues])
    if available_fixes:
...
def apply_fixes(content, fixes):
    changed_lines = set()
    content_map = {idx: line for idx, line in enumerate(content.splitlines())}
    messages = list()
    for fix in fixes:
        line_changes = set(
...
async def create_pull_request(repository, task_id, source_branch, pull_request, updates):
    ...
    ref = repo.create_git_ref(ref=f"refs/heads/{branch_name}", sha=repo.get_branch(branch=source_branch).commit.sha)
    pr_messages = [
    ...
    for file, update in updates.items():
        ...
        pr_messages.append(
            f"Fixed {update['issue_count']} {'issue' if len(update['issue_count']) == 1 else 'issues'} in {file}")
    pr = repo.create_pull(
